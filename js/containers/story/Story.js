...
import { getRedditComments } from "../../network/api";
...
export class Story extends Component {
  ...
  componentDidMount = async () => {
    ...
    const isReddit = story.category.startsWith("r/");
  ...
  fetchRedditComments = async () => {
    const { story } = this.props.navigation.state.params;
    try {
      const comments = await getRedditComments(story.id);
      this.setState({
  ...
  handleRefresh = () => {
    const { kids, category } = this.props.navigation.state.params.story;
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <CommentList
          comments={values(
            isReddit ? this.state.redditComments : commentsByHash
          )}
          kids={story.kids}
          ...
          hasErrored={isReddit ? this.state.hasRedditErrored : hasErrored}
        />
        {story && (
          <ShareStory
            story={story}
